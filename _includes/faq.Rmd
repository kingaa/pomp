...
bibliography: ../vignettes/pomp.bib
csl: ../vignettes/jss.csl    
---
...
## I am using **R** version X. When I try to install the package, I get the error that "package **pomp** is not available (for R version X)". Yet it is stated that **pomp** is compatible with R(&ge;Y), where X > Y. What is going wrong?
...
Also, some Windows users using C snippets in parallel computations report errors (e.g., [issue 120](https://github.com/kingaa/pomp/issues/120)) that appear to be triggered when the temporary files created during C snippet compilation are created or destroyed.
This problem does not appear to crop up on other platforms.
A workaround for this involves compiling all codes *before* parallel computations commence and using the `cdir` and `cfile` arguments---which can be passed to `pomp` or any of the **pomp** elementary algorithms or inference algorithms---to cause the C snippets to be compiled in the directory, and with the file name, of your choice.
...
## When trying to use C snippets, I get an error message that refers to files and directories that don't exist. What is going on?
...
Error: error in building shared-object library from C snippets: in ‘Cbuilder’: compilation error: cannot compile shared-object library ...
compiler messages:
...
warning: "beta" redefined
```

The error you got is just due to a name conflict between a parameter `beta` and the beta function, which is defined in `Rmath.h` and [documented in the R extensions manual](https://cran.r-project.org/doc/manuals/r-release/R-exts.html#Mathematical-functions).
...
## What are "parameter transformations" (`partrans`, `parameter_trans`) in **pomp** and when should I use them?
...
See the `parameter_trans` manual page for more detail.

## Iterated filtering (`mif2`)

### How should I choose the random-walk intensities (`rw.sd`)?
...
See the manual page for `rw.sd` for details.
