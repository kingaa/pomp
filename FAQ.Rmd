...
require(knitr)
require(pomp)
knitr::opts_chunk$set(cache=FALSE,results="hide",echo=TRUE,fig.show="hide",
...
## Why can't I run the demos, or other codes that use the `Csnippet` facility?

<a id="csnippet-problems"></a>

If your system does not support `R CMD SHLIB`, you won't be able to use `Csnippets`.
...
```
To do all of the above, download and run the `mac-fortran.sh` script:
```
