...
theta <- coef(ou2)
theta["alpha_2"] <- 0.1
pf1 <- pfilter(pf,params=theta,Np=100,tol=0)
stopifnot(identical(coef(pf1),theta))

try(pfilter(ou2,params=parmat(coef(ou2),100),tol=0))
try(pfilter(ou2,Np=100,tol=0,params=parmat(coef(ou2),100)))

pf2 <- pfilter(ou2,Np=function(k)c(100,150)[(k-1)%%2+1],tol=0)
table(pf2@Np)
...
pf1 <- pfilter(pf,save.states=TRUE,filter.traj=TRUE,tol=0)
pf2 <- pfilter(pf,pred.mean=TRUE,pred.var=TRUE,filter.mean=TRUE,tol=0)
pf3 <- pfilter(pf,t0=1,filter.traj=TRUE,tol=0)
pf4 <- pfilter(pf,dmeasure=Csnippet("lik = (give_log) ? R_NegInf : 0;"),
  filter.traj=TRUE,tol=0)
...
try(ou2 %>% as.data.frame() %>% pfilter(Np=1000,tol=0))
