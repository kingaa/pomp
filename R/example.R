...
##'
##' @param \dots additional arguments define symbols in the environment within which the example code is executed.
##'
##' @param show logical;
##' if \code{TRUE}, display, but do not execute, the example code.
##'
##' @param envir the environment into which the objects should be loaded.
...
pompExample <- function (example, ..., show = FALSE, envir = .GlobalEnv) {
  ...
  if (example=="") {
    ...
    file <- c(lapply(exampleDirs,list.files,
      ...
      recursive=TRUE)
    if (length(file)<1) {
      pStop(ep,"cannot find file ",sQuote(paste0(example,".R")))
    }
    if (length(file)>1) {
     pWarn(ep,"using ",sQuote(file[1])," from ",sQuote(names(file)[1]))
    }
    if (show) {
      file.show(file[1])
    ...
    if (is.null(envir)) {
      ...
      pStop(ep,sQuote("envir")," must be an environment or NULL")
    }
    invisible(obj)
  }
}
