...
spect.match <- function(object, start, est = character(0),
    ...
    if (!is(object,"pomp"))
        stop(ep,sQuote("object")," must be of class ",sQuote("pomp"),call.=FALSE)

    if (missing(start)) start <- coef(object)

    if (!eval.only&&(length(est)<1))
        stop(ep,"parameters to be estimated must be specified in ",sQuote("est"),call.=FALSE)
    if (!is.character(est)|!all(est%in%names(start)))
        stop(ep,sQuote("est")," must refer to parameters named in ",sQuote("start"),call.=FALSE)
    ...
    if (missing(nsim)) {
        if (is(object,"spect.pomp"))
            ...
            stop(ep,sQuote("nsim")," must be supplied",call.=FALSE)
    }
