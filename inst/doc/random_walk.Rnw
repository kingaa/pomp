...
The codes above show how the \code{pomp} object is constructed.
Being written in R, however, they're not particularly fast.
To maximize computational efficiency, we'll instead use compiled codes.
For more information on this topic, see the vignette, ``Using compiled code in \code{pomp}''.
<<>>=
data(ou2)
@ 
