...
Lower-level access is available via the \code{rprocess} method:
<<>>=
...
@ 
Note that \code{dprocess} returns a matrix: the rows correspond to independent simulations, the columns to distinct state transitions.
...
The following wrapper functions make use of these compiled codes.
<<>>=
  ou2.rprocess <- function (xstart, times, params, ...) 
...
These will be passed to each of the functions and will come in handy later when we do particle filtering.
...
@ 
In this case, the Kalman filter gives us a log likelihood of \code{fit2\$loglik=\Sexpr{fit2$loglik}}, while the particle filter gives us \code{fit1\$loglik=\Sexpr{fit1$loglik}}.
...
For this example, we'll just draw our particles from a multivariate normal distribution.
<<>>=
...
Now we'll run MIF to maximize the likelihood over two of the parameters and the initial conditions.
...
coef(fit)
@
The log likelihood of the random-parameter model at the end of the mif iterations, which should be a rough approximation of that of the fixed-parameter model, is \code{logLik(fit)=}\Sexpr{logLik(fit)}.
