...
You are always welcome to ask for help, but maintainer time is limited.
Therefore, before submitting a request, first see if you can solve your own problem.
The following steps will help you to do so.

1. Check to see if you are using [the latest version of **pomp**](https://cran.r-project.org/package=pomp):
if not, check to see if the problem you have is solved by [upgrading to the latest version](https://kingaa.github.io/pomp/install.html).
1. Before submitting a request, examine the error messages and try to figure out what they are telling you by reading the [manual](./manual/) and/or the [package tutorials](./docs.html).
...
If you find that your question has not been answered, or you believe you have found a bug, then submit a request for help via the [Issues page](https://github.com/kingaa/pomp/issues) if possible or to Aaron King (kingaa at umich dot edu) if necessary.
...
However, you should know that each time the major version number for the package increases (e.g., from 5.11 to 6.1), at least one change has been introduced that has the potential to break existing code.
...
See the [installation instructions](./install.html#important-note-for-mac-os-users).
...
For example, look at the following variant of [the SIR model describing the influenza outbreak in a boarding school](https://kingaa.github.io/sbied/stochsim/):
...
sir_step <- Csnippet("
  double dN_SI = rbinom(S,1-exp(-β*I/N*dt));
...
[What is the probability of seeing something if you do not look?]
...
```{r}
sir_na |>
  pfilter(Np=1000,params=c(β=3,γ=2,ρ=0.9,N=2600)) |>
  as.data.frame() |>
  pivot_longer(-day) |>
...
As described in the documentation ([`?pomp`](https://kingaa.github.io/manuals/pomp/html/pomp.html#:~:text=contains the names of accumulator variables)), **pomp** allows you to define "accumulator variables" that collect events occurring between observations.
...
When constructing a `pomp` object for a compartmental model using the Euler multinomial approximation, to write the rprocess basic component, one uses the `euler` plugin.
...
Note that the rules for `step.fun` C snippets guarantee that `dt` will be the operative time step ($\Delta{t}$).
...
See also the documentation for the [**pomp** C API](https://kingaa.github.io/pomp/C_API.html#euler-multinomial-distribution).
...
The [manual page](https://kingaa.github.io/manuals/pomp/html/eulermultinom.html) contains more information on the Euler-multinomial distribution.
...
See the help ([`?dmeasure_spec`](https://kingaa.github.io/manuals/pomp/html/dmeasure_spec.html)) for information on how to properly write `dmeasure` C snippets.
...
Remember, `dmeasure` has to be able to return both the likelihood and the log likelihood upon request!
...
An illustration is given in the [Simulation-based Inference for Epidemiological Dynamics](https://kingaa.github.io/sbied/misc/windows.html) short course.
...
Whenever your model needs the value of a covariate at a particular time, it uses the lookup table provided (via the [`covar`](https://kingaa.github.io/manuals/pomp/html/pomp.html#:~:text=optional covariate table) argument), linearly interpolating if necessary.
...
For example, it could be called `Beta` or `β` (if your compiler allows it) instead.

## I get an error saying X "must be a function of the form" Y.  What does this mean?

The error message displays the "signature" of the function **pomp** expects.
For example, in the message
```{r eval=FALSE}
Error : in 'sannbox': 'candidate.dist' must be a function of the form 'candidate.dist(par, temp, scale, ...)'.
```
the signature expected is `candidate.dist(par, temp, scale, ...)`.
...
(a) the full vector of model parameters has been supplied neither via the `params` argument nor in the `pomp` object's `coef` slot;
(b) a basic model component that computes state variables (e.g., `rinit` or `rprocess`) is not returning the full vector of state variables;
(c) the covariate table does not contain the full set of covariates; or
(d) some data variable is missing.

It may seem puzzling when, for example, the `rinit` workhorse complains about the lack of a parameter upon which the rinit basic component does not in fact depend.
To understand this, it is necessary to know that, whenever one or more C snippets are passed to a **pomp** function, the names of the parameters, state variables, covariates, and observables that are needed by *any* of them will be expected by *all* of them.
Thus, when `rinit` complains that it cannot find a certain parameter that it actually doesn't need to know, it is effectively doing so on behalf of another basic model component which will depend on that parameter.

Finally, note that, while one informs **pomp** functions about the names of parameters using the `paramnames` argument and about the names of state variables using `statenames`, the names of covariates are by default drawn from the covariate table itself, and the names of the observables are taken from the data.
This behavior can be over-ridden using the optional `covarnames` and `obsnames` arguments.
...
[When [parameter transformations](#partrans) are in use, the perturbations will be applied on the estimation scale.]
...
The `rw_sd` function allows one to specify (via simple **R** expressions in terms of time `time`) which parameters fall into this category and when and how much they are to be perturbed.
...
Note that a call to `Rprintf` involves passing a quoted string.
To avoid having to escape those quotes, and the linefeed character `"\n"`, as in the above code, one can use [**R**'s raw string literals](https://r4ds.hadley.nz/strings.html#sec-raw-strings), as in the following.
