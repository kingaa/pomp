...
\Abstract{
  ...
  \noindent
  *This document is an updated version of \href{http://dx.doi.org/10.18637/jss.v069.i12}{\emph{Journal of Statistical Software} \textbf{69}(12): 1--43}.
...
obs(ex_gompertz)
states(ex_gompertz)
as.data.frame(ex_gompertz)
plot(ex_gompertz)
timezero(ex_gompertz)
time(ex_gompertz)
coef(ex_gompertz)
@
...
\begin{algorithm}[t!]
  ...
  \For {$m$ in $\seq{1}{M}$}{
    ...
    $[\Theta^F_{0,j}]_i \sim \normal\left([\theta_{m-1}]_i, (C a^{m-1} \sigma_i)^2\right)$ for $i$ in $\seq{1}{p}$, $j$ in $1\myto J$.\;
    ...
    $\bar \theta_0=\theta_{m-1}$.\;
    Define $[V_1]_i=(C^2+1)(a^{m-1}\sigma_i)^2$.\;
    \For {$n$ \textrm{in} $1\myto N$}{
      ...
      $[V_{n+1}]_i=(a^{m-1}\sigma_i)^2 + \sum_{j}\tilde{w}(n,j)\big([\Theta^P_{n,j}]_i - [\bar{\theta}_{n}]_i\big)^2$ for $i \not\in I$.\;
    }
    Update non-IVPs: \nllabel{alg:mif:update}
    $[\theta_m]_i=[\theta_{m-1}]_i+ [V_1]_i \sum_{n=1}^{N} [V_{n}]_i^{-1}\left([\bar{\theta}_{n}]_{i}-[\bar{\theta}_{n-1}]_{i}\right)$ for $i \not\in I$.\;
...
When the function \code{gompertz.meas.dens} is called, \code{y} will contain the observation at time \code{t}, \code{x} and \code{params} will be as before, and the parameter \code{log} will indicate whether the likelihood (\code{log = FALSE}) or the log likelihood (\code{log = TRUE}) is required.
...
<<pfilter2-calc,eval=F,purl=F>>=
 pf <- pfilter(gompertz,params=coef(gompertz),Np=1000)
@
or simply
<<pfilter3-calc,eval=F,purl=F>>=
...
gompertz <- pomp(gompertz, params=coef(gompertz), partrans = parameter_trans(toEst = gompertz.log.tf, fromEst = gompertz.exp.tf))
@
...
theta.true <- theta
<<gompertz-mif-inner1,echo=F,eval=F,purl=F>>=
theta.guess <- theta.true
theta.guess[estpars] <- rlnorm(n = length(estpars),
    meanlog = log(theta.guess[estpars]), sdlog = 1)
<<gompertz-mif-inner2,echo=F,eval=F,purl=F>>=
mif2(gompertz, Nmif = 100, params = theta.guess,
    Np = 2000, cooling.fraction = 0.7,
    rw.sd = rw.sd(r=0.02, sigma=0.02,tau=0.02))
<<gompertz-mif-inner3,echo=F,eval=F,purl=F>>=
pf <- replicate(n = 10, logLik(pfilter(mf2, Np = 10000)))
logmeanexp(pf)
<<gompertz-mif-estpar,eval=F,purl=F,echo=F>>=
estpars <- c("r", "sigma", "tau")
<<gompertz-mif-post,eval=F,purl=F,echo=F>>=
...
<<gompertz-mif-demo,eval=F,purl=F,echo=T,tidy=F>>=
<<gompertz-mif-estpar>>
library("foreach")
...
<<pmcmc-inner,eval=F,purl=F,echo=F>>=
...
<<pmcmc-demo,eval=F,purl=F,echo=T,tidy=F>>=
...
<<pmcmc-diagnostics,results="hide",fig.show="hide",echo=F,eval=F,purl=F>>=
...
<<first-probe-plot,eval=F,purl=F>>=
summary(pb.truth)
summary(pb.guess)
plot(pb.truth)
plot(pb.guess)
...
<<ricker-probe-match-calc,eval=F,purl=F,results="markup">>=
pm <- probe.match(
                  pb.guess,
                  est=c("r","sigma","phi"),
                  transform=TRUE,
...
<<ricker-mif-calc,eval=F,purl=F,tidy=FALSE>>=
...
<<abc-load,eval=F,purl=F,echo=F,tidy=F>>=
...
<<abc-diagnostics,results="hide",fig.show="hide",echo=FALSE,eval=FALSE,purl=F>>=
...
<<first-nlf,eval=F,purl=F,tidy=F>>=
...
We do this by furnishing a custom \code{initializer} in the call to \code{pomp}.
...
Further documentation and an introductory tutorial are provided with the package and on the \pkg{pomp} website, \url{http://kingaa.github.io/pomp}.
