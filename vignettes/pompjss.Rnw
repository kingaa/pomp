...
\newcommand\nprobe{k}
\newcommand\Nprobe{K}
\newcommand\carryingCapacity{\kappa}
...
\item The additional variability can be exploited to explore the likelihood surface and estimate the gradient of the (smoothed) likelihood, based on the same filtering procedure that is required to estimate the value of the likelihood \citep{Ionides2006,Ionides2011}.
...
Neither normality nor independence are necessary for iterated filtering to converge to a neighborhood of the MLE \citep{Ionides2015}.
However, rather than varying the perturbation distribution, it is convenient to transform the parameters to make these simple algorithmic choices reasonable.
\code{pomp} provides a \code{partrans} argument to facilitate optimization on a suitably transformed parameter space.
...
In general, a parameter should be perturbed only during a time interval where it influences the transition or measuremnt process.
...
\begin{algorithm}[t!]
  \caption{
    ...
    \texttt{pmcmc(P, params{\argequals}$\theta_0$, Nmcmc{\argequals}$M$, Np{\argequals}$J$, proposal{\argequals}mvn.rw($V$))},
    using notation from \cref{tab:notation}
...
Other plug-in functions include \code{mvn.diag.rw} and \code{mvn.rw.adaptive}.
...
\begin{algorithm}[t!]
  ...
  $s_{{\nprobe}j}=\featureFunc_{\nprobe}(Y^j_{1:N})$ for  ${\nprobe}$ in $1\myto {\Nprobe}$ and $j$ in $\seq{1}{J}$.\;
  Compute sample mean:
  $\mu_{\nprobe}=J^{-1}\sum_{j=1}^Js_{{\nprobe}j}$ for  ${\nprobe}$ in $\seq{1}{\Nprobe}$.\;
  Compute sample covariance:
  $V_{i{\nprobe}}=(J-1)^{-1}\sum_{j=1}^J (s_{ij}-\mu_i)(s_{{\nprobe}j}-\mu_{\nprobe})$ for $i$ and ${\nprobe}$ in $\seq{1}{\Nprobe}$.\;
  Compute the log synthetic likelihood:
  \begin{equation}
    \loglikMC_{\featureFunc}(\theta)=-\frac{1}{2}\,(s^*-\mu)^{\top}{V}^{-1}(s^*-\mu)-\frac{1}{2}\,\log|V|-\frac{d}{2}\,\log(2\pi).
  \end{equation}\;
...
\begin{algorithm}[t!]
  \caption{
    ...
    \protect
    \texttt{abc(P, params{\argequals}$\theta_0$, Nmcmc{\argequals}$M$, probes{\argequals}$\featureFunc$, scale{\argequals}$\tau_{1:\Nprobe}$, proposal{\argequals}mvn.rw($V$), epsilon{\argequals}$\epsilon$)},
...
%    simulator for $q(\theta^P\given\theta)$;
  ...
  \KwOut{
    Samples, ${\theta_{1:M}}$, representing the posterior distribution, $f_{\Theta|S_{1:{\Nprobe}}}(\theta\given s_{1:{\Nprobe}}^*)$.
  }
