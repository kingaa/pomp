...
\noindent
*This document is a version of a manuscript in press at the \emph{Journal of Statistical Software}.
...
NLF, by contrast, uses simulations to fit a linear model which predicts $Y$ at time $n$ using its value at lags $\lags_1,\dots,\lags_L$, i.e., a linear regression of $Y_n$ on the $L$ variables $\{Y_{n-\lags_1},\dots,Y_{n-\lags_L}\}$.
...
\clearpage
\subsection{Adding realistic complexities}
