...
replicate(10,logLik(pfilter(gompertz,params=theta.mif,Np=10000))) %>%
...
loglik.truth <- -kalman(coef(gompertz),gompertz,coef(gompertz))
loglik.mif <- -kalman(theta.mif,gompertz,coef(gompertz))
...
theta.mle <- coef(gompertz)
theta.mle[estpars] <- kalm.fit1$par
loglik.mle <- -kalm.fit1$value

## ----gompertz-post-mif2,include=FALSE------------------------------------
replicate(n=10,logLik(pfilter(gompertz,Np=10000))) %>%
  logmeanexp(se=TRUE) -> pf.loglik.truth
replicate(n=10,logLik(pfilter(gompertz,params=theta.mle,Np=10000))) %>%
