...
For ordinary splines, one passes the spline knots in `knots`, together with the degree (`degree`) desired.
Note that `knots` must point to an array of length at least `nbasis + degree + 1`.
The first form evaluates the `nbasis` B-spline basis functions at `x`, the values being returned in the array pointed to by `y`.
...
The first form evaluates `nbasis` periodic B-spline basis functions at `x`, the values being returned in the array pointed to by `y`.
The second evaluates the order-`deriv` derivative of each basis function.
...
That is, $r = \log{(1+R\,\Delta{t})}/{\Delta{t}}$ is the rate for an Euler process that gives the same expected waiting time as the exponential process it approximates.
