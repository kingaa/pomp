...
- approximate Bayesian computation (ABC) of e.g., @Toni2009
...
The counting processes are coupled to the state variables [@breto11] via the following identities
$$\begin{aligned}
...
If we are willing to live with an approximate simulation scheme, we can use the so-called "tau-leap" algorithm, one version of which is implemented in **pomp** via the `euler.sim` plug-in.
...
$$(\dlta{\cp{S}{I}},\dlta{\cp{S}{}},S-\dlta{\cp{S}{I}}-\dlta{\cp{S}{}})\sim\mathrm{Multinom}\left(S(t);p_{SI},p_{S\BirthDeath},1-p_{SI}-p_{S\BirthDeath}\right),$$
where
$$\begin{aligned}
...
The help page (`?euler.sim`) informs us that to use `euler.sim`, we need to specify a procedure that advances the states from $t$ to $t+\dlta{t}$.
...
We cause **pomp** to do this via the `pomp` function's `zeronames` argument, as we will see in a moment.
...
The **pomp** function `probe.match` attempts to maximize this likelihood.
...
In the above, one has a choice of optimization algorithm (via the `method` argument).

One can also construct a self-contained objective function via a call to `probe.match.objfun` and use this in an arbitrary optimization routine. 
...
- probe-matching methods [@Kendall1999;@Wood2010]
- the nonlinear forecasting method [@Ellner1998;@Kendall2005]
- the ensemble Kalman filter of [@Evensen1994;@Evensen2009]
- the ensemble adjustment Kalman filter of @Anderson2001a
- power-spectrum-matching methods of @Reuman2006.
...
## [**R** codes for this document](oaxaca.R)
## [**Rmarkdown** source code for this document](oaxaca.Rmd)
## [**pomp** homepage](https://kingaa.github.io/pomp/)

--------------------

This document draws on [materials from the "Simulation-based Inference for Epidemiological Dynamics" course](https://kingaa.github.io/sbied/) taught by E. L. Ionides and A.A.K. and also on @King2016.
