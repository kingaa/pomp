...
![schematic1](http://kingaa.github.io/images/pomp_schematic1.png)

-----------------------------------

Here is another POMP model schematic, showing dependence among model variables.

![schematic2](http://kingaa.github.io/images/pomp_schematic2.png)
...
$$P_t \sim \mathrm{Poisson}(N),$$
where $P_t$ is our census population size at time $t$ and $N$ is the true population size.
...
At each of these (`foreach`), we first run 50 iterations of the IF2 algorithm.
We then re-run the algorithm, starting from where the first run ended up.
...
To complete the inference, we do `r nrow(traces)` more MCMC iterations using a multivariate random-walk proposal (`mvn.rw`) with covariance matrix derived from the preceding computation (`covmat`).
...
Now let us simulate from the posterior median parameters.
```{r pmcmc-sim}
...
For example, the following codes run a particle filter at a sample of points from the posterior distribution (assuming that the PMCMC chains above have converged) and extract a single trajectory from each one.
Collectively, as @Andrieu2010 showed, this is a sample from the smoothing distribution.
```{r parus-filter-traj}
