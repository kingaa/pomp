...
opts_chunk$set(
  ...
  size='small',
  fig.lp="fig:",
  fig.path=paste0("tmp/figure/",params$prefix,"/"),
  cache.path=paste0("tmp/cache/",params$prefix,"/"),
  fig.pos="h!",
  fig.align='center',
...
library(DT)
dfprintfn <- function (x, ...) {
  x |>
    DT::datatable() |>
    knitr::knit_print()
}
registerS3method("knit_print", "data.frame", dfprintfn)
```
...
```{r parallel_setup,include=FALSE,purl=TRUE,cache=FALSE}
...
Finally, note that we received a warning about \code{NA} values being generated.
...
```{r vp_loglik,eval=FALSE,purl=FALSE}
replicate(10,
  vpC |>
...
ll_vp1
```
```{r vp_loglik_eval,include=FALSE,purl=TRUE}
bake("vp_loglik.rds",{
  <<vp_loglik>>
}) -> ll_vp1
ll_vp1
...
```{r vp_mif1,eval=FALSE,purl=FALSE}
vpC |>
  mif2(
    params=guesses[1,],
    Np=1000,
...
```
```{r vp_mif1_eval,include=FALSE,purl=TRUE}
bake("vp_mif1.rds",{
  <<vp_mif1>>
}) -> mf1
```
