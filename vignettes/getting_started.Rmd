...
Internally, **circumstance** uses the **doFuture** package to parallelize computations.
...
These are set using the `plan()` function.
**doFuture** also provides parallel random-number generators, so that we can consider each of the parallel computations to be independent.
