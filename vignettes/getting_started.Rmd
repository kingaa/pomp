...
output:
  html_document:
    ...
    df_print: paged
params:
  prefix: "getting-started2"
  min.pomp.version: "2.0.9"
  show_solns: TRUE
...
This document was produced using **pomp2** version `r packageVersion("pomp2")` and **R** version `r getRversion()`.
...
This tutorial aims to help you get started using **pomp2** as a suite of tools for analysis of time series data based on stochastic dynamical systems models.
First, we give some conceptual background regarding the class of models---partially observed Markov processes (POMPs)---that **pomp2** handles.
We then discuss some preliminaries: 
installing the package and so on.
Next, we show how to simulate a POMP using **pomp2**.
We then analyze some data using a few different tools.
In so doing, we illustrate some of the package's capabilities by using its algorithms to fit, compare, and criticize the models using **pomp2**'s algorithms.
...
As its name implies **pomp2** is focused on partially observed Markov process models.
...
```{r ssdiag2,echo=FALSE,purl=FALSE,fig.height=3,fig.width=6,fig.cap="**Conditional independence graph of a POMP.**  The latent state $X_n$ at time $t_n$ is conditionally independent of its history given $X_{n-1}$.  The observation $Y_n$ is conditionally independent of all other variables given $X_n$.  The underlying time scale can be taken to be either discrete or continuous, and the observation times need not be equally spaced."}
library(grid)
...
To implement a POMP model in **pomp2**, we have to specify the measurement and process distributions.
...
At present, **pomp2** is focused on such methods, so there is no reason to discuss the dprocess component further in this document.
In the following, we will illustrate and explain how one specifies the rprocess, rmeasure, and dmeasure components of a model in **pomp2**.
...
To get started, we must install **pomp2**, if it is not already installed.
...
Compilers are not by default installed on Windows or Mac systems, so users of such systems must do a bit more work to make use of **pomp2**'s facilities.
...
Having dispensed with the preliminaries, we now explore some of the functionality provided by **pomp2**.
...
To accomplish this in **pomp2**, as we've already discussed, we specify the rprocess component of the model.
...
Names of variables and parameters are very important in **pomp2**.
...
```{r sim1_spy,eval=FALSE}
spy(sim1)
```

**pomp2** provides methods for plotting 'pomp' objects.
For example,
...
Now notice that we had a warning when we ran `simulate`.
The warning was telling us that `simulate` could not simulate from the measurement process because we had supplied it with no rmeasure component.
Let's see how we can specify rmeasure, the measurement model simulator.
...
**pomp2** uses certain syntactic conventions.
For example, if `x` is an object and `f` is a **pomp2** function, then typically `f(x,...) -> y` yields an object, `y`, that contains `x` plus the results of the `f` operation, together with additional information pertinent to what `f` did.
...
The [**magrittr** package](https://cran.r-project.org/package=magrittr) (part of the [**tidyverse**](https://cran.r-project.org/package=tidyverse)), provides new syntax that is especially convenient for us.
...
In contrast, the pipeline syntax of **magrittr** allows us to write
```
x %>% f(...) %>% g(...) %>% h(...)
```
...
Of course, it is not necessary to adopt this style of programming to use **pomp2**, but it is quite natural and, experience shows, quite addictive!

We will use pipelines freely for the remainder of the document.
Note that it is not necessary to load **magrittr** to use this syntax:
the `%>%` operator is imported from **magrittr** and re-exported by **pomp2**.

***Important note:*** 
If you opt to load the **tidyverse** (or individual packages therein), be sure to load **pomp2** *after* loading these packages.
There are some name conflicts between the packages that would otherwise cause **pomp2** functions to be masked.
...
We will illustrate some of the **pomp2** data-analysis algorithms by performing a limited analysis on a set of bird abundance data.
...
The basic data-type provided by **pomp2**---the 'pomp' object---is designed as a container for a model and data.
...
To implement this model in **pomp2**, we must tell the package how to simulate this model.
...
**pomp2** gives us the `euler` function to assist us in implementing the Euler-Maruyama method.
...
```{r vp1,eval=FALSE,include=FALSE}
parus %>%
  pomp(
    times="year", t0=1960,
    rinit=function (N_0, ...) {
...
The `rnorm` and `rpois` functions are part of the [**R** API](http://cran.r-project.org/doc/manuals/r-release/R-exts.html#The-R-API):
see the [manual on "Writing R Extensions"](http://cran.r-project.org/doc/manuals/r-release/R-exts.html) for a description of these and the other [distribution functions provided as part of the **R** API](http://cran.r-project.org/doc/manuals/r-release/R-exts.html#Distribution-functions).
A full set of rules for writing **pomp2** C snippets is given in the package help (`?Csnippet`).
...
Here, `dpois` again comes from the [**R** API](http://cran.r-project.org/doc/manuals/r-release/R-exts.html#Distribution-functions).
...
This powerful algorithm is at the heart of several of **pomp2**'s inference methods.
...
The [**pomp2** *Journal of Statistical Software*](https://kingaa.github.io/pomp/vignettes/pompjss.pdf) paper gives pseudocode and some examples.
The [**pomp2** documentation page](https://kingaa.github.io/pomp/docs.html) lists several other tutorial documents that go into more detail.

In **pomp2**, the simplest version of the particle filter is implemented in the function `pfilter`.
...
Although **pomp2**'s main focus is on stochastic models, it does provide facilities for trajectory matching.
**pomp2** makes a conceptual distinction between the stochastic process and a *deterministic skeleton*, which we can view as a deterministic model related to the stochastic process' central tendency.
...
We implement this for **pomp2**, again either as an **R** function or a C snippet and pass it to **pomp2** functions via the `skeleton` argument.
For example:
```{r rick_skel}
rickC %>%
  pomp(
...
In **pomp2**, the function `traj_objfun` constructs an objective function quantifying the mismatch between model predictions and data.
...
'traj_match_objfun' objects, like the objective functions created by the **pomp2** functions `spect_objfun`, `probe_objfun`, and `nlf_objfun`, is an **R** function, but it is also more than an **R** function.
...
We incorporate parameter transformations using the `partrans` argument to many **pomp2** functions, specifying them using the `parameter_trans` function.
...
We could then provide `vpr_partrans` as needed to any of the various **pomp2** inference methods, via the `partrans` argument.
...
Let us now turn to the main focus of the **pomp2** package:
...
An [addendum to the **pomp** *Journal of Statistical Software paper](http://kingaa.github.io/pomp/vignettes/mif2.html) provides pseudocode and a simple example.
Finally, a [tutorial on the theory and practice](https://kingaa.github.io/sbied/mif/mif.html) is linked from the [**pomp2** documentation index](http://kingaa.github.io/pomp/docs.html).
...
We use the `sobolDesign` function to construct an overdispersed sequence of points within the selected box.

```{r mif_guesses,fig.width=4,fig.height=4}
sobolDesign(
...
Now, from each of these starting guesses, we will run the IF2 algorithm (implemented as `mif2` in **pomp2**).
...
**pomp2** provides various methods for 'mif2d_pomp' objects.
...
```{r parus_mif1,purl=FALSE,eval=FALSE}
library(foreach)
...
```
```{r parus_mif1_eval,include=FALSE,eval=TRUE,purl=TRUE}
...
```{r parus_pf1,purl=FALSE,eval=FALSE}
foreach (mf=mifs,
...
```
```{r parus_pf1_eval,include=FALSE,eval=TRUE,purl=TRUE}
...
```{r mif_plot4,warning=FALSE,fig.width=4,fig.height=4}
estimates %>%
  full_join(guesses) %>%
...
A good first choice is **doParallel**, which is a separate package that you must explicitly load.
...
```{r parus_mif2,purl=FALSE,eval=FALSE}
estimates %>%
  filter(!is.na(loglik)) %>%
...
```
```{r parus_mif2_eval,include=FALSE,eval=TRUE,purl=TRUE}
...
```{r mif2_plot1,fig.width=4,fig.height=4}
estimates %>%
...
```{r parus_pd,fig.width=4,fig.height=4}
estimates %>%
...
```
```{r parus_profile,eval=FALSE,purl=FALSE}
foreach (start=iter(starts,"row"),
  .combine=rbind, .packages=c("pomp2"),
  .errorhandling="remove", .inorder=FALSE) %dopar% {
...
```
```{r parus_profile_eval,include=FALSE,purl=TRUE,eval=TRUE}
...
**pomp2** provides a number of tools to facilitate answering these questions through interaction with a fitted model.
...
Let's perform this operation using several of the summary statistics provided with **pomp2**:
...
```{r probe1_plot,fig.width=6.8,fig.height=6.8}
plot(vp_probe)
```
...
The [**pomp** website](https://kingaa.github.io) contains more documentation, including the full package manual, and a variety of tutorials and short courses.
The package itself contains a number of built-in examples and datasets that can be explored.

**pomp2** provides a large toolbox of different inference methods, only a few of which have been explored here.
...
The [bibliography](http://kingaa.github.io/pomp/biblio.html) has links to peer-reviewed publications that have used these methods.

## [**R** codes for this document](getting_started.R)
