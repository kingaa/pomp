...
gompertz <- pomp(
                 ...
                 parameter.inv.transform=function(params,...){
                   params <- c(
                               params["X.0"],
                               exp(params[c("log.r","log.K","log.tau","log.sigma")])
                               )
...
@ 
To be clear: the parameter-setting expression assumes that \code{theta} is a named vector of parameters on the natural scale.
After this operation, the parameters are stored internally in \code{gompertz}, on the internal scale, as we can check by doing
<<>>=
coef(gompertz)
@ 
...
<<par-trans-inverse-test,results=verbatim>>=
