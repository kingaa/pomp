...
init.state(gompertz)
@
Executing \code{pompExamples()} lists other examples provided with the package.
...
\subsection{Full-information Bayesian inference via PMCMC}\label{sec:gompertz:pmcmc}
...
\subsection{A second example: the Ricker model}\label{sec:ricker:setup}
...
\subsection{Feature-based synthetic likelihood maximization}\label{sec:ricker:probe.match}
...
\subsection{Bayesian feature matching via ABC} \label{sec:gompertz:abc}
...
\subsection{Parameter estimation by simulated quasi-likelihood}\label{sec:gompertz:nlf}
...
The package contains more examples (via \code{pompExamples}), which can be used as templates for implementation of new models;
