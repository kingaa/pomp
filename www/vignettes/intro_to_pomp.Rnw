...
\textbf{NB: For all but the very simplest models, it is almost always necessary to use compiled native codes to make the computationally intensive algorithms in \pkg{pomp} fast enough to be useful in practice.}
